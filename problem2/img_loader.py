...
class IMGProcess(object):
    ...
    def tensor2img(self, tensor, index, name):
        ...
        for t in tensor:
            ...
            score = round((t[-2]).item()*100,2)
            #color = random.choice(self.colors)
            color = 'red'
            draw.rectangle([s_x, s_y, e_x, e_y], outline=color,width=2)
            draw.text([s_x, s_y], label+':'+str(score)+'%', fill='black', font=self.font)
          
        del draw

        img.save(os.path.join(self.result, "res_{}".format(name)),dpi=(300.0,300.0))
